...
const errorHandler = function(res) {
    ...
    if(res.config.loading){Vue.$_loading.hide()}
    ...
    // 非空字符串即有标题
    res.config.title ? Vue.$toast.fail(res.config.title, _msg) : Vue.$toast.fail(_msg);
    // 抛出错误
...
methods.forEach(m => {
     ...
     */
    ajax[m] = function(url, data = {}, { timeout = 10000 ,loading = false} = {}) {        
        ...
        if(config.loading){Vue.$_loading.show()}
